...
On a strict hold-out test period (2021Q1--2024Q4), rolling refits yield RMSE of 3.87 and MAE of 1.58---higher than univariate ARIMA(0,1,1) (RMSE 3.05) but comparable when excluding Argentina (RMSE 3.22 vs.\ 2.83), while retaining all 23 countries that break the VAR(4) panel.
Cluster-robust Diebold--Mariano tests do not reject equal accuracy at the 5\% level ($p=0.12$ vs.\ ARIMA), and a TOST equivalence test with a 10\% margin does not support equivalence ($p=0.64$).
...
Seed-robustness checks were not run because of computational cost and are left for future work.
...
We also report a Two One-Sided Test (TOST) for equivalence with a pre-specified margin of 10% of the ARIMA RMSE from the apples-to-apples 22-country comparison (Argentina excluded), testing whether the ex-Argentina RMSE difference falls within this margin.

\section{Results}
\label{sec:results}

Table~\ref{tab:benchmark} summarizes out-of-sample accuracy on the 2021--2024 test period.
The rolling ST-GNN achieves RMSE of 3.87 and MAE of 1.58---higher than ARIMA (RMSE 3.05, MAE 1.34) but comparable when excluding Argentina (RMSE 3.22 vs.\ 2.83), while retaining all 23 countries that break VAR estimation.
...
\input{generated/table1.tex}

\input{generated/table_apples.tex}
...
A TOST equivalence test with a 10\% margin based on the ex-Argentina ARIMA RMSE does not support equivalence between ST-GNN and ARIMA ($p=0.64$).

\input{generated/table2.tex}

\input{generated/table_tost.tex}
...
\textbf{Robustness checks:} We verify that attribution rankings are not merely tracking trade volume by comparing shock periods (2020Q2, 2022Q2) to a calm baseline period (2017Q2).
For Germany, the top 3 trade partners during COVID were China, Netherlands, and USA; during the calm 2017Q2 baseline they were China, France, and Netherlands.
The partial overlap (2/3 partners: China, Netherlands) is suggestive, but it should be read as a limited robustness check rather than definitive evidence, because the exercise was performed for Germany only and does not yet cover the Indonesia case that motivates the paper's strongest claim.
Seed-robustness checks were not run because of computational cost and are left for future work.
...
This case is consistent with the contagion hypothesis: for open emerging economies, monitoring partner-country inflation and trade-link intensity is as important as domestic CPI when forecasting cost-push pressures, although the robustness exercise above is limited to Germany and should therefore be interpreted as suggestive rather than definitive.
...
On a 2021--2024 hold-out with rolling refits, the model achieves RMSE of 3.87---higher than univariate ARIMA but comparable when excluding Argentina (RMSE 3.22 vs.\ 2.83) while encoding trade structure that panel VARs cannot accommodate without excluding countries and variables.
...
The replication code, data processing scripts, and processed datasets are available at: \url{https://github.com/mrayanasim09/MarketEquationDiscovery}.
