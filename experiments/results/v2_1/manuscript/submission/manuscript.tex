...
graph ranks first at h = 1 (1.707 pp). However, no graph model achieves
statistically significant superiority over its best non-graph comparator
in a majority of seeds after FDR correction, suggesting that the
temporal attention mechanism rather than trade-network topology drives
any observed gains. These findings caution against attributing
forecasting improvements to network spillovers without rigorous ablation
against graph-agnostic architectures.
\end{abstract}
...
\subsection{6.3 Statistical Robustness}\label{statistical-robustness}
